...
/**
 * @file
 * Custom JavaScript for Event Registration module.
 */
...
(function ($, Drupal) {
  ...
  /**
   * Enhanced form interactions for event registration.
   */
  Drupal.behaviors.eventRegistration = {
    attach: function (context, settings) {
      
      // Add loading class to form during AJAX requests
      $('.event-registration-form', context).once('event-registration').each(function () {
        var $form = $(this);
      ...
      if ($('.messages--error', context).length) {
        $('html, body').animate({
          scrollTop: $('.messages--error').offset().top - 100
        }, 500);
      }

      // Add confirmation for form submission
      $('.event-registration-form .form-submit', context).once('submit-confirm').on('click', function (e) {
        var $form = $(this).closest('form');
        var email = $form.find('input[name="email"]').val();
        
        if (email && !$(this).hasClass('confirmed')) {
          e.preventDefault();
          var confirmed = confirm('Are you sure you want to register with email: ' + email + '?');
          if (confirmed) {
      ...
      $('input[type="email"]', context).once('email-format').on('blur', function () {
        var email = $(this).val().trim().toLowerCase();
        $(this).val(email);
      });

      // Character counter for text fields (optional enhancement)
      $('.event-registration-form input[type="text"]', context).once('char-counter').each(function () {
        var $input = $(this);
      ...
      $('.event-registration-form .required', context).once('required-highlight').on('focus', function () {
        $(this).closest('.form-item').addClass('is-focused');
      }).on('blur', function () {
        $(this).closest('.form-item').removeClass('is-focused');
      });

      // Admin listing: Update URL with filter parameters
      $('.registration-filters select', context).once('filter-params').on('change', function () {
        var eventDate = $('select[name="event_date"]').val();
        var eventName = $('select[name="event_name"]').val();
        
        var params = [];
        if (eventDate) params.push('event_date=' + eventDate);
        if (eventName) params.push('event_name=' + eventName);
      ...
      $('.registration-table tr', context).once('row-highlight').on('mouseenter', function () {
        $(this).addClass('highlighted');
      }).on('mouseleave', function () {
        $(this).removeClass('highlighted');
      });

      // Print functionality for admin listing
      if ($('.registration-table', context).length) {
        var $printBtn = $('<button type="button" class="button print-button">Print List</button>');
        $('.registration-table', context).before($printBtn);
  ...
  /**
   * Custom AJAX command for updating participant count.
   */
  Drupal.AjaxCommands.prototype.updateParticipantCount = function (ajax, response, status) {
    $('.participant-count').html('<strong>Total Participants: ' + response.count + '</strong>');
  };
